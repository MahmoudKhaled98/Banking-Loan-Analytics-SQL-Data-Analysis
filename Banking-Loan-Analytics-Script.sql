...
SELECT * 
FROM Bank_Personal_Loan_Modelling bplm
WHERE Age IS NULL 
...
DELETE FROM Bank_Personal_Loan_Modelling
WHERE rowid NOT IN (
    SELECT MIN(rowid)
...
SELECT 
    ...
    sqrt((SUM(Income * Income) / COUNT(Income)) - (AVG(Income) * AVG(Income))) AS StdDev_Income
FROM 
...
SELECT 
    ...
    AVG(CCAvg) AS Avg_CCAvg, 
    MIN(CCAvg) AS Min_CCAvg, 
    MAX(CCAvg) AS Max_CCAvg
FROM 
    Bank_Personal_Loan_Modelling bplm;

-- Insight: Avg_Age: 45.34, Min_Age: 23, Max_Age: 67, Avg_Income: 73.77, Min_Income: 8, Max_Income: 224, Avg_CCAvg: 1.94.
...
SELECT 
    `Personal Loan`, 
    COUNT(*) AS Count, 
    COUNT(*) * 100.0 / (SELECT COUNT(*) FROM Bank_Personal_Loan_Modelling bplm) AS Percentage
FROM 
    Bank_Personal_Loan_Modelling bplm 
GROUP BY  
    `Personal Loan`;
